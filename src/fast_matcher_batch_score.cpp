// PA02 최적화: Score() GPU 배치화
// 15번 개별 GPU 호출 → 1번 배치 GPU 호출
// score_all_batch() 사용: 모든 scan의 candidate를 한 번에 처리
...
static void WriteLog(int call, double score_ms, double branch_ms, double total_ms) {
  static bool header = false;
  const char* path = "/tmp/pa02_batch_score_runtime.csv";
  FILE* f = std::fopen(path, "a");
  if (!f) return;
  if (!header) {
    std::fprintf(f, "call,score_ms,branch_ms,total_ms\n");
    header = true;
  }
  std::fprintf(f, "%d,%.6f,%.6f,%.6f\n", call, score_ms, branch_ms, total_ms);
  std::fclose(f);
}
...
// ── Score: GPU 배치화 ─────────────────────────────────────────────────────────
// 15번 개별 GPU 호출 → all_px/all_py/scan_id 빌드 → score_all_batch() 1번 호출
...
bool FastMatcher::MatchWithWindow(const std::vector<float>& xs,
  ...
  *out = MatchOut();
  ...
  ++call_id;
  WriteLog(call_id, score_ms, branch_ms, total_ms);
  std::fprintf(stderr, "[pa02_batch_score] call=%d score=%.3fms branch=%.3fms total=%.3fms\n",
               call_id, score_ms, branch_ms, total_ms);
